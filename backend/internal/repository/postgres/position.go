...
func (r *PositionRepo) Get(ctx context.Context, req *models.GetPositionDTO) ([]*models.Position, error) {
	...
	query := fmt.Sprintf(`SELECT id, count, name, note, amount, is_deleted, updated_at, created_at,
		COALESCE(sum_made, 0) AS made, COALESCE(sum_acc, 0) AS accepted, COALESCE(sum_rejected, 0) AS rejected, COALESCE(sum_sent, 0) AS sent, amount<=COALESCE(sum_made, 0) as is_done, 
		...
		LEFT JOIN LATERAL (SELECT SUM(amount) AS sum_rejected FROM %s WHERE position_id=positions.id) AS rejected ON true
		...
		PositionsTable, MadeTable, AcceptedTable, RejectsTable, SentTable, sort,
	)
