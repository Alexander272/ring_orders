...
func (r *UserRepo) GetAll(ctx context.Context) ([]*models.UserData, error) {
	query := fmt.Sprintf(`SELECT id, username, email, sso_id, first_name, last_name FROM %s ORDER BY last_name`, UsersTable)
...
func (r *UserRepo) GetById(ctx context.Context, id string) (*models.UserData, error) {
	query := fmt.Sprintf(`SELECT id, username, email, sso_id, first_name, last_name FROM %s WHERE id=$1`, UsersTable)
	data := &models.UserData{}
...
func (r *UserRepo) GetBySSOId(ctx context.Context, id string) (*models.UserData, error) {
	query := fmt.Sprintf(`SELECT id, username, email, sso_id, first_name, last_name FROM %s WHERE sso_id=$1`, UsersTable)
	data := &models.UserData{}
...
func (r *UserRepo) Create(ctx context.Context, dto *models.UserData) error {
	query := fmt.Sprintf(`INSERT INTO %s(id, username, email, sso_id, first_name, last_name) VALUES ($1, $2, $3, $4, $5, $6)`, UsersTable)
...
func (r *UserRepo) CreateSeveral(ctx context.Context, dto []*models.UserData) error {
	query := fmt.Sprintf(`INSERT INTO %s(id, username, email, sso_id, first_name, last_name) VALUES `, UsersTable)
...
func (r *UserRepo) Update(ctx context.Context, dto *models.UserData) error {
	query := fmt.Sprintf(`UPDATE %s SET username=$1, email=$2, sso_id=$3, first_name=$4, last_name=$5 WHERE id=$6`, UsersTable)
...
func (r *UserRepo) UpdateSeveral(ctx context.Context, dto []*models.UserData) error {
	...
	query := fmt.Sprintf(`UPDATE %s AS t SET username=s.username, email=s.email, first_name=s.first_name, last_name=s.last_name 
		...
		UsersTable, strings.Join(values, ","),
	)
...
func (r *UserRepo) DeleteSeveral(ctx context.Context, ids []string) error {
	query := fmt.Sprintf(`DELETE FROM %s WHERE id=ANY($1)`, UsersTable)
