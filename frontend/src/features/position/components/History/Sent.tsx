...
import { useGetSentQuery } from '../../sentApiSlice'

type Props = {
	id: string
}

export const Sent: FC<Props> = ({ id }) => {
	const { data, isFetching } = useGetSentQuery(id || '', { skip: !id })
			...
			<TableBody>
				{data?.data.map(d => (
					<TableRow key={d.id} height={36} hover sx={{ cursor: 'default' }}>
						...
						<TableCell width={360}>{numberFormat(d.quantity)}</TableCell>
					</TableRow>
				))}
			</TableBody>
		</Table>
